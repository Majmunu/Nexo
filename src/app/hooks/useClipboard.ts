...
// 定义 ClipboardData 类型，表示剪贴板存储的数据结构
// 支持任意类型的数据，不再强制要求 name 标识
...
interface UseClipboard {
    copyToClipboard: <T = any>(displayText: string, data: FlexibleClipboardData<T>) => Promise<void>
    pasteFromClipboard: <T = any>() => Promise<FlexibleClipboardData<T> | string | null>
    isSupported: boolean
...
export function useClipboard(): UseClipboard {
    ...
    const hasPermission = useCallback(async (): Promise<boolean> => {
        ...
        try {
            ...
            return writePermission.state !== 'denied'
        } catch {
    ...
    // 数据验证函数 - 移除了 name 属性的验证
    const validateInput = useCallback(<T = any>(displayText: string, data: FlexibleClipboardData<T>): void => {
    ...
    // 创建包装数据
    const createWrappedData = useCallback(<T = any>(data: FlexibleClipboardData<T>) => ({
    ...
    // 复制到剪贴板 - 使用 useCallback 优化性能
    const copyToClipboard = useCallback(async <T = any>(
        displayText: string,
        data: FlexibleClipboardData<T>
    ): Promise<void> => {
        if (!isSupported) {
    ...
    // 从剪贴板粘贴 - 使用 useCallback 优化性能
    const pasteFromClipboard = useCallback(async <T = any>(): Promise<FlexibleClipboardData<T> | string | null> => {
        if (!isSupported) {
            throw new ClipboardError('Clipboard API is not supported in this environment', 'UNSUPPORTED')
        }
